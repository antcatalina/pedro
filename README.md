...
- **Sandboxed by default.** `check` runs generated code in an isolated subprocess with a wall-clock timeout (backstopped by a POSIX CPU-time limit inside the child, so it bites even if the parent's timer is starved), so an agent-authored infinite loop is reported as `status: "timeout"` instead of hanging your CI or your laptop.
...
| `files`    | `read file "<path>"`, `write <text> to file "<path>"` | **implemented** |
...
exits `0`; a disagreement (or any lane failing) exits `1`. Capability programs run on
**both** backends — the TypeScript lane emits the same adapter calls against a
reference `pedro_capabilities.ts`, so they are cross-checked like any other program.
Passing a single target
...
- **Next (highest priority first)** — the remaining capability verbs (`http`/`time`/`random`) + modules (`use "file.pedro"`). (db `update`/`delete` and the `files` capability (`read file`/`write … to file`) both **landed** — the database capability now has the full `insert`/`find`/`update`/`delete` CRUD surface on both backends, and `files` round-trips through the reference adapter.)
